...
class _PresetListState extends State<PresetList> {
  ...
  Widget build(BuildContext context) {
    ...
    print("열린 타일 그룹 아이디 :  $expandedKey");
    ...
    return ListView(
        ...
        children: _presetModel.groupList.map((ele) {
          ...
          return Container(
              margin:  EdgeInsets.only(bottom: SizeUtil().sh01), // 마진 줄 수 있음
              // height: SizeUtil().sh20,
              decoration: BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.circular(SizeUtil().sh05),
                boxShadow: [
                  BoxShadow(
              ...
              child: ExpansionTile(
            initiallyExpanded: groupId == 0, // todo 초기값 수정
            key: Key('$groupId'),
            ...
            title: expandedKey.contains(groupId) // 타이틀 부분
              ...
              children: [
                Text(groupName,style: TextStyle(fontSize: SizeUtil().sh05/2, fontWeight: FontWeight.bold),),
                Row(
                  children: [
                    SizedBox(
                      width: SizeUtil().sh05,
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(groupName,style: TextStyle(fontSize: SizeUtil().sh05/2, fontWeight: FontWeight.bold,color: Colors.blueGrey),),
                ]
            ), // 타이틀 부분(닫힘)
            leading: expandedKey.contains(groupId) ? Icon(
              MaterialCommunityIcons.folder_open_outline,
              color: Colors.blueGrey,
              size: SizeUtil().sh075/2,
            ) : Icon(
